...
class AppDelegate: NSObject,
    ...
    func applicationDidFinishLaunching(_ notification: Notification) {
        ...
        // Add the "New Workspace Window" item to the File menu (opt-in feature).
        installWorkspaceMenuItem()
    ...
    func applicationDidBecomeActive(_ notification: Notification) {
        ...
        if !applicationHasBecomeActive {
            ...
            if TerminalController.all.isEmpty && WorkspaceController.all.isEmpty && derivedConfig.initialWindow {
                undoManager.disableUndoRegistration()
                // Opt-in: if the workspace is configured as the default window,
                // open it instead of a classic terminal window. Defaults off so
                // standard Ghostty behavior is unchanged unless the user opts in.
                if AppDelegate.workspaceIsDefaultWindow {
